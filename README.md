...
터미널(Terminal.app)에서 아래 두 줄이면 끝 — 최초 실행 시 의존성은 자동 설치됩니다.
...
> 💡 API 키 없이도 동작합니다(무료 로컬 모드). `export ANTHROPIC_API_KEY=sk-...` 후 실행하면 Claude가 더 정확히 묶어줍니다.
>
> ⚠️ 자동 업데이트를 받으려면 **`git clone` 으로 설치**해야 합니다(zip 다운로드 ❌).

### 다른 실행 방법

```bash
./run.sh --browser            # 앱을 브라우저로 실행
./run.sh cli scan ~/Desktop   # CLI 로 스캔
./run.sh cli groups           # CLI 로 그룹 보기
```
...
앱을 열면 백그라운드로 원격(git)과 비교해 새 버전이 있으면 **상단 배너**로 알립니다. `업데이트` 를 누르면 `git pull` 후 자동 재시작합니다. (git 저장소로 clone 한 경우만)
