...
1. **[최신 릴리스에서 `shotsort.dmg` 다운로드](https://github.com/sj48695/shotsort/releases/latest)**
2. `.dmg` 를 열고 **shotsort 를 Applications 폴더로 드래그**
3. 첫 실행만: Launchpad/Finder 에서 앱 **우클릭 → 열기 → "열기"**
   (서명 안 된 빌드라 첫 회만 macOS 경고를 우회합니다. 이후엔 더블클릭)
...
**`.app` 설치**: 새 버전은 [릴리스 페이지](https://github.com/sj48695/shotsort/releases/latest)에서 최신 `.dmg` 를 받아 교체하세요. (번들 자동 업데이트는 추후 지원 예정)
