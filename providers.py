...
_SECRET_PATTERNS = (
    ...
    (re.compile(r"(?i)\b((?:OPENAI|ANTHROPIC|XAI)_(?:API_KEY|AUTH_TOKEN))=[^\s,;]+"), r"\1=***"),
)
...
def resolve_execution(mode: AnalysisMode | str = AnalysisMode.AUTO,
    ...
    if selected_mode in (AnalysisMode.AUTO, AnalysisMode.CLI) and capability and capability.ready_for(with_image=with_image):
        # 자동 추천은 CLI 자체의 기본 모델 선택에 맡긴다.
        cli_model = None if selected_mode == AnalysisMode.AUTO else selected_config.model
        cli_config = ProviderConfig("codex", cli_model)
    ...
    if selected_mode in (AnalysisMode.AUTO, AnalysisMode.CLI) and "claude" not in capabilities:
